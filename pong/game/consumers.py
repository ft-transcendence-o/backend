...
class GameConsumer(WebsocketConsumer):
    ...
    def init_game(self):
        self.ball_pos = np.array([0, 0, 0]) #공위치  #@
        self.ball_vec = np.array([0, 0, 2]) #공이 움직이는 방향
        self.ball_rot = np.array([0, 0, 0]) #공의 회전벡터
        self.angular_vec = np.array([0, 0, 0])
        # self.flag = True # 공이 날라가는 방향
        self.panel1_pos = np.array([0, 0, 50]) #panel1의 초기위치 #@
        self.panel2_pos = np.array([0, 0, -50]) #panel2의 초기위치 #@
    ...
    def plane_distance_to_point(self, plane):
        ...
        return abs(ballX * a + ballY * b + ballZ * c) / math.sqrt(a**2 + b**2 + c**2)
...
# 구가 평면과 부딪힌 좌표
    ...
    def collisionInPanel(self, panelPlane):
        ...
        #충돌지점 계산
        collisionPoint = get_collision_point_with_plane(panelPlane)

        # 충돌후 공의 좌표를 보정
        self.ball_pos = collisionPoint + panelPlane[0] * 2

        # angularVec = panelPlane[0] * 0.01
        updateVector(panelPlane)
        #updateVectorByPanel()
